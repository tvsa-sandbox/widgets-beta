...
const Video = props => {
    ...
    const changeActive = name => {
        console.log("estoy dentro", name);
        if (name === "CLIPS" && clipActive) return null;
        if (name === "EPISODIOS" && episodeActive) return null;
        if (name === "COMMENTS" && episodeActive) return null;
        if (name === "EPISODIOS" && (clipActive || commentActive)) {
            return setState({
                episodeActive: true,
                clipActive: false,
                commentActive: false,
        ...
        if (name === "CLIPS" && (episodeActive || commentActive)) {
            return setState({
                clipActive: true,
                episodeActive: false,
                commentActive: false,
        ...
        if (name === "COMMENTS" && (episodeActive || clipActive)) {
            return setState({
                commentActive: true,
                clipActive: false,
                episodeActive: false,
    ...
    return (
        <Layout breadcrumbs={[]} social={socialmedia}>
            <PageStyles>
                ...
                <TabPane idtab="comentarios" active={commentActive}>
                    comentarios
                </TabPane>
            </PageStyles>
        </Layout>
